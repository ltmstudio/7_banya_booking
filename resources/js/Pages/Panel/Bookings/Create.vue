<script setup>
/**
 * Форма создания бронирования: комната, клиент/гость, дата, время, доп. услуги.
 */
...
/** Выбранная комната по form.room_id для отображения лимита гостей. */
...
/** Опции длительности брони из настроек (min_hours..max_hours). */
const durationOptions = computed(() => {
    return buildHalfHourOptions(props.bookingSettings.min_hours, props.bookingSettings.max_hours);
});

/** Подпись длительности в селекте: "N ч" / "N ч 30 мин". */
...
const form = useForm({
    ...
    booking_date: '',
    start_time: '10:00',
    duration_hours: props.bookingSettings.min_hours,
    guests_count: 1,
    extra_service_ids: [],
    guest_name: '',
    guest_phone: '',
...
const submit = () => {
    form.post(route('panel.bookings.store'));
};
...
<template>
    <DashboardLayout>
        <Head :title="t('bookings.create')" />
        <template #header>
            <div class="flex items-center justify-between gap-4">
                <h1 class="text-xl font-semibold text-gray-800">{{ t('bookings.create') }}</h1>
                <Link :href="route('panel.bookings.index')" class="text-sm text-gray-500 hover:text-orange-600">← {{ t('common.cancel') }}</Link>
            </div>
        </template>
        ...
        <div class="mx-auto max-w-2xl px-4 pb-12 pt-8">
            <form class="rounded-2xl border border-gray-200 bg-white p-8 shadow-md" @submit.prevent="submit">
                <div class="space-y-4">
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.room') }} <span class="text-orange-500">*</span></label>
                        <select
                        ...
                        >
                            <option value="">—</option>
                            <option v-for="r in rooms" :key="r.id" :value="r.id">{{ roomName(r) }}</option>
                        </select>
                        <p v-if="form.errors.room_id" class="mt-1 text-xs text-red-500">{{ form.errors.room_id }}</p>
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.client') }}</label>
                        <select
                            v-model="form.client_id"
                            class="w-full rounded-lg border border-gray-200 px-4 py-2.5 text-sm focus:border-orange-300 focus:outline-none focus:ring-2 focus:ring-orange-400/20"
                        >
                            <option value="">{{ t('bookings.client_guest_option') }}</option>
                            <option v-for="c in clients" :key="c.id" :value="c.id">{{ c.full_name }} ({{ c.phone }})</option>
                        </select>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.booking_date') }} <span class="text-orange-500">*</span></label>
                            <input
                        ...
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.start_time') }} <span class="text-orange-500">*</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.duration_hours') }} <span class="text-orange-500">*</span></label>
                            <select
                                v-model="form.duration_hours"
                                class="w-full rounded-lg border border-gray-200 px-4 py-2.5 text-sm focus:border-orange-300 focus:outline-none focus:ring-2 focus:ring-orange-400/20"
                            >
                                <option v-for="h in durationOptions" :key="h" :value="h">{{ durationLabel(h) }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.guests_count') }} <span class="text-orange-500">*</span></label>
                            <input
                                ...
                                class="w-full rounded-lg border border-gray-200 px-4 py-2.5 text-sm focus:border-orange-300 focus:outline-none focus:ring-2 focus:ring-orange-400/20"
                            />
                            <p v-if="selectedRoom?.capacity" class="mt-1 text-xs text-gray-500">
                                {{ t('bookings.max_guests') }}: {{ selectedRoom.capacity }}
                            </p>
                            <div
                    ...
                    <div v-if="extraServices.length">
                        <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.extra_services') }}</label>
                    ...
                    <p class="text-xs font-medium uppercase tracking-wider text-gray-500">{{ t('bookings.guest_section_heading') }}</p>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.guest_name') }}</label>
                            <input v-model="form.guest_name" type="text" maxlength="150" :placeholder="t('bookings.guest_name_placeholder')" class="input-field" />
                        </div>
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.guest_phone') }}</label>
                            <PhoneInput v-model="form.guest_phone" :error="form.errors.guest_phone" />
                        </div>
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.guest_last_name') }}</label>
                        ...
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-700">{{ t('bookings.guest_birthday') }}</label>
                            <BirthdayInput v-model="form.guest_birthday" :error="form.errors.guest_birthday" />
                        </div>
                    </div>
                </div>
                <div class="mt-8 flex justify-end gap-3 border-t border-gray-200 pt-8">
                    <Link :href="route('panel.bookings.index')" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                        {{ t('common.cancel') }}
                    </Link>
                    <button
                        type="submit"
